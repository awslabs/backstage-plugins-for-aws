...
export const createGetPipelinesByEntityAction = ({
  awsCodePipelineApi,
  actionsRegistry,
}: {
  ...
  actionsRegistry.register({
    name: `get-aws-codepipelines`,
    title: `Get AWS CodePipeline Summary`,
    attributes: {
      destructive: false,
      readOnly: true,
      idempotent: true,
    ...
    description: `
This allows you to retrieve the state of the AWS CodePipeline pipelines related to a single entity from the software catalog.
...
Each entity is identified by a unique entity reference, which is a string of the form "kind:namespace/name".
    ...
    action: async ({ input, credentials }) => {
      const response = await awsCodePipelineApi.getPipelineStateByEntity({
        entityRef: input,
        credentials,
      });
