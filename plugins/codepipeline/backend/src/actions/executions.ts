...
import { AwsCodePipelineService } from '../service';

export const createGetPipelineExecutionsByEntityAction = ({
  awsCodePipelineApi,
  actionsRegistry,
}: {
  ...
  actionsRegistry.register({
    name: `get-aws-codepipeline-executions`,
    title: `Get AWS CodePipeline Executions`,
    attributes: {
      destructive: false,
      readOnly: true,
      idempotent: true,
    ...
    description: `
This allows you to retrieve executions for AWS CodePipeline pipelines related to a single entity from the software catalog.
...
Each entity is identified by a unique entity reference, which is a string of the form "kind:namespace/name".
    ...
    schema: {
      input: z =>
        z.object({
          ...
          arn: z.string().describe('AWS CodePipeline ARN'),
          pageSize: z
            .number()
            .describe('Number of pipeline executions to return in a page')
            .default(5)
            .optional(),
          page: z
            .number()
    ...
    action: async ({ input, credentials }) => {
      const response =
        await awsCodePipelineApi.getPipelineExecutionsByEntityWithArn({
          entityRef: input,
          credentials,
          arn: input.arn,
      ...
      return {
        output: {
          executions: response.pipelineExecutions,
        },
      };
    },
  });
